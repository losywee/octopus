...
- ⚖️ **负载均衡** - 自动分配请求，确保服务稳定高效，内置熔断器与重试/故障转移
- 🔄 **协议互转** - 支持 OpenAI Chat / OpenAI Responses / Anthropic 三种 API 格式互相转换
- 🚫 **Key 级模型列表** - 每个 API 密钥可配置允许（白名单）/ 禁止（黑名单）模型列表
- 💰 **价格同步** - 自动更新模型价格
- 🔃 **模型同步** - 自动与渠道同步可用模型列表，支持站点根地址的多路径 `/v1/models` 探测
- 📊 **数据统计** - 全面的请求统计、Token 消耗（含缓存读写 Token）、费用追踪、客户端 IP 记录、按分组/模型排行
- 🎨 **优雅界面** - 简洁美观的 Web 管理面板
